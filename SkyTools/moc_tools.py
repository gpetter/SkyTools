...
def make_unwise_bitmask(max_order, low_nside):
	...
	hp.write_map('masks/area_lost_%s.fits' % low_nside, area_lost_map, overwrite=True)
...
def combined_area_lost_mask(nside):
	contamlist = ['xsc', 'pne', 'HII', 'lvg']
	orders = [14, 15, 15, 14]

	lost_area = hp.read_map('masks/area_lost_%s.fits' % nside)
